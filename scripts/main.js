...
function convertCSVArrayToTraineeData(csvArrays) {
  trainees = csvArrays.map(function(traineeArray, index) {
    ...
    trainee.top9 = traineeArray[5] === 't'; // sets trainee to top 11 if 't' appears in 6th column
    trainee.id = parseInt(traineeArray[6]) - 1; // trainee id is the original ordering of the trainees in the first csv
    trainee.image =
      trainee.name_romanized.replace(" ", "").replace("-", "").replace("/", "").replace(" ","") + ".png";
    return trainee;
  });
...
const currentURL = "https://lyzfan.github.io/youth3/";
